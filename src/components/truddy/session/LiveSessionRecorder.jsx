...
function PastRecordingCard({ session, isExpanded, onToggle }) {
  ...
  return (
    ...
    >
      ...
      <div className="px-4 py-3 flex items-center justify-between">
        ...
        <div className="ml-3 text-[#faf9f6] opacity-40">
          ...
          >
            ...
            {session.full_transcript && (
              <div className="pt-3 space-y-1">
                ...
                <div
                  className="text-xs leading-relaxed text-[#faf9f6] opacity-70 max-h-28 overflow-y-auto"
            ...
            {session.problem_identified && (
              <div className="rounded-[12px] p-3 space-y-1"
                style={{ background: "rgba(255,100,80,0.1)", border: "1px solid rgba(255,100,80,0.25)" }}>
                <div className="flex items-center gap-1.5 text-[10px] uppercase tracking-wider font-semibold"
                  style={{ color: "rgba(255,120,90,0.9)" }}>
                  <AlertTriangle size={11} /> Biggest Problem
                </div>
                <div className="text-xs leading-relaxed text-[#faf9f6] opacity-80">
                  {session.problem_identified}
                </div>
              </div>
            )}
            ...
            {session.proposed_solution && (
              <div className="rounded-[12px] p-3 space-y-1"
                style={{ background: "rgba(80,220,160,0.08)", border: "1px solid rgba(80,220,160,0.25)" }}>
                <div className="flex items-center gap-1.5 text-[10px] uppercase tracking-wider font-semibold"
                  style={{ color: "rgba(80,220,160,0.9)" }}>
...
export default function LiveSessionRecorder({ onClose }) {
  ...
  const processRecording = async (audioBlob) => {
    ...
    // AI analysis for problem + solution
    const analysis = await base44.integrations.Core.InvokeLLM({
      prompt: `You are a trading psychology coach. Analyze this trader's session transcript and identify:
1. The single biggest psychological problem or behavioral pattern you observe.
2. A clear, actionable solution or mental reframe to address it.
